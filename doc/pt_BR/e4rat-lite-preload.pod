...
Primeiro, ele lê as informações de I-Node dos arquivos, depois ele lê o conteúdo dos arquivos. Isso pode ser feito paralelamente com a execução do arquivo especificado em --execute. Se o e4rat-lite-preload for executado como um processo de inicialização pela adição dele aos parametros do kernel, o /sbin/init (ou o processo indicado na configuração) é chamado.
